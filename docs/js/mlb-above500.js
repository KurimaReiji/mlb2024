import { MlbTeams as Teams, teams_by_wpct, to_total, waitFor, load_css_rules, load_font, svgdownload } from "./mlb2024.js";
...
const get_title = (league, division, year = 2024) => `Games above .500, ${league} ${division} ${year}`;
...
function tweet({ league, division, series }) {
  const url = "https://github.com/KurimaReiji/mlb2024";
  const title = get_title(league, division);
  const tw = series
...
async function downloadHandler(detail) {
  ...
  const waitForFonts = [
    { name: "Noto Sans Mono", url: "/mlb2024/fonts/NotoSansMono-Regular.ttf" },
...
class MlbAbove500 extends HTMLElement {
  ...
  render() {
    const css = `<link rel="stylesheet" href="/mlb2024/css/mlb2024-colors.css">
    <style>
    :host {
      --padding-left: 50;
      --padding-right: 140;
